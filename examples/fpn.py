"""FPN with a MobileNetV2 encoder, 21 classes - a feature-pyramid decoder.

    python examples/fpn.py

Nothing here is DelegateDoctor-specific. This file builds a stock
`segmentation_models_pytorch` FPN and hands it to the same public
`optimize()` that any user calls:

    from delegate_doctor import optimize
    optimize(model, args=(example_input,))

DelegateDoctor has no idea what a FPN is. It exports whatever model it is
...
def build_model():
    """A stock smp FPN, built exactly as the recorded evidence was."""
    torch.manual_seed(0)
    return smp.FPN(
