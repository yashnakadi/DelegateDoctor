"""PSPNet with a MobileNetV2 encoder, 21 classes - pyramid pooling; the largest measured DD-001 win.

    python examples/pspnet.py

Nothing here is DelegateDoctor-specific. This file builds a stock
`segmentation_models_pytorch` PSPNet and hands it to the same public
`optimize()` that any user calls:

    from delegate_doctor import optimize
    optimize(model, args=(example_input,))

DelegateDoctor has no idea what a PSPNet is. It exports whatever model it is
...
def build_model():
    """A stock smp PSPNet, built exactly as the recorded evidence was."""
    torch.manual_seed(0)
    return smp.PSPNet(
