"""Demo workload: PSPNet with a MobileNetV2 encoder, 21 classes.

Architecture:  PSPNet (pyramid scene parsing decoder)
...
Run:  delegate-doctor doctor pspnet
"""

from delegate_doctor.export_model import ModelSpec
from delegate_doctor.models import build_model_spec, create_pspnet


def create_model():
    """The bare PSPNet, if you want it without DelegateDoctor's wrapper."""
    return create_pspnet()


def build_model() -> ModelSpec:
    """What `delegate-doctor doctor pspnet` loads."""
    return build_model_spec("pspnet")
